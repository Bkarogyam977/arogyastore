...
//         <h2 className="text-xl font-bold text-gray-800">Full Body Checkup Packages</h2>
//         <Link href="/#">
...
const FullBodyCheckupPackages = () => {
  ...
  return (
    <div className="w-full p-4 bg-white rounded-lg">
      ...
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl md:text-2xl font-bold text-gray-800">Full Body Checkup Packages</h2>
        <Link href="/#">
      ...
      <div className="relative">
        <div className="flex gap-3 md:gap-5 overflow-x-auto pb-4 scroll-container">
          {packages.map((pkg) => (
            <div key={pkg.id} className="flex-shrink-0 w-48 md:w-64 lg:w-72 package-card bg-white rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-all">
              <div className="p-2 md:p-3">
                <div className="relative h-32 md:h-40 rounded-md overflow-hidden">
                  <Image
                    src={pkg.image}
                    alt={pkg.name}
                    fill
                ...
                <div className="mt-2 md:mt-3 px-1">
                  <h3 className="text-xs md:text-sm font-semibold text-gray-800 line-clamp-2">{pkg.name}</h3>
                  <p className="text-[10px] md:text-xs text-gray-500 mt-1">Contains {pkg.tests} Tests</p>
                  <p className="text-[10px] md:text-xs text-green-600 mt-1">Reports in {pkg.reportTime}</p>
                  
                  <div className="flex items-center justify-between mt-1 md:mt-2">
                    <div>
                      <span className="text-xs md:text-sm font-bold text-gray-900">₹{pkg.discountPrice}</span>
                      <span className="text-[10px] md:text-xs text-gray-500 line-through ml-1">₹{pkg.price}</span>
                    </div>
                    <div className="flex items-center">
                      <FaStar className="text-yellow-400 text-[10px] md:text-xs" />
                      <span className="text-[10px] md:text-xs text-gray-700 ml-1">{pkg.rating}</span>
                    </div>
                  </div>
                  
                  <button className="w-full mt-2 md:mt-3 py-1.5 md:py-2 bg-blue-600 hover:bg-blue-700 text-white text-[10px] md:text-xs font-semibold rounded transition-colors">
                    Book Now
                  </button>
                </div>
              </div>
