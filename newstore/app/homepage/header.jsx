...
import { Drawer, Button, Dropdown, Menu, Badge, Collapse } from "antd";
...
import {
  ...
  ShoppingOutlined, ShopOutlined, InfoCircleOutlined,
  FileTextOutlined, AppstoreOutlined, FireOutlined
...
const HeaderSection = ({ domainuser }) => {
  ...
  const closeDrawer = () => {
    setOpen(false);
    // Reset all dropdowns when closing drawer
    setOpenDropdowns({
      "shop-by-category": false,
      "best-seller": false,
      "treatments": false
  ...
  // Sample categories for mobile view
  const shopCategories = [
    { label: "Hair-Care", path: "/e-store/categoryproduct/328" },
    { label: "Immunity-Boosters", path: "/e-store/categoryproduct/235" },
    { label: "Sexual-Wellness", path: "/e-store/categoryproduct/244" },
    { label: "womwen-care", path: "/e-store/categoryproduct/328" },
    { label: "Stomach-care", path: "/e-store/categoryproduct/239" },
  ];

  // Sample best sellers for mobile view
  const bestSellers = [
    { label: "Slim-Tea", path: "/e-store/productdetails/2512" },
    { label: "Green-Tea", path: "/e-store/productdetails/2651" },
    { label: "Shilajit", path: "/e-store/productdetails/3136" },
    { label: "Multivitamin", path: "/e-store/productdetails/1003" },
    { label: "Nasha-Mukti", path: "/e-store/productdetails/1217" },
  ];

  // Updated menuItems according to NavBar
  const menuItems = [
    {
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#f2f9f1] lg:bg-[#5b974f]">
      <div className="flex justify-between items-center px-2 lg:px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            onClick={() => router.push("/")}
            className="cursor-pointer mx-2 my-2 pl-8 lg:pl-0 text-xl  lg:text-2xl font-bold capitalize text-gray-800 lg:text-white hover:text-gray-500 transition-colors duration-300"
          >
      ...
      >
        <div className="flex flex-col h-full overflow-y-auto">
          <ul className="flex-grow">
              ...
              if (item.showAlways) {
                return (
                  <li key={index} className="border-b border-gray-100">
                    <div className="flex items-center justify-between px-6 py-4 text-gray-800 hover:text-green-600 hover:bg-gray-50 transition-colors cursor-pointer"
                         onClick={() => {
                           if (item.hasDropdown) {
                             toggleDropdown(item.dropdownName);
                           } else {
                    ...
                    >
                      ...
                      <div className="flex items-center">
                        {item.badge && (
                          <span className="mr-2 px-2 py-1 text-xs bg-gradient-to-tr from-pink-500 to-yellow-500 text-white rounded-lg">
                            {item.badge}
                          </span>
                        )}
                        {item.hasDropdown && (
                          <DownOutlined 
                            className={`transition-transform duration-300 ${openDropdowns[item.dropdownName] ? 'rotate-180' : ''}`}
                          />
                        )}
                      </div>
                    </div>
                    ...
                    {item.hasDropdown && item.dropdownName === "shop-by-category" && openDropdowns["shop-by-category"] && (
                      <div className="bg-gray-50 border-t border-gray-100">
                        <ul>
                          {shopCategories.map((category, catIndex) => (
                            <li key={catIndex} className="border-b border-gray-100 last:border-b-0">
                              <Link
                                href={category.path}
                                className="flex items-center px-12 py-3 text-gray-600 hover:text-blue-600 hover:bg-gray-100 transition-colors"
                                onClick={closeDrawer}
                              >
                    ...
                    {item.hasDropdown && item.dropdownName === "best-seller" && openDropdowns["best-seller"] && (
                      <div className="bg-gray-50 border-t border-gray-100">
                        <ul>
                          {bestSellers.map((seller, sellerIndex) => (
                            <li key={sellerIndex} className="border-b border-gray-100 last:border-b-0">
                              <Link
                                href={seller.path}
                                className="flex items-center px-12 py-3 text-gray-600 hover:text-blue-600 hover:bg-gray-100 transition-colors"
                                onClick={closeDrawer}
            ...
            {/* Treatment Section with Dropdown */}
            <li className="border-b border-gray-100">
              <div 
                className="flex items-center justify-between px-6 py-3 font-medium text-gray-800 cursor-pointer hover:bg-gray-50"
                onClick={() => toggleDropdown("treatments")}
              >
                <span>Treatments</span>
                <DownOutlined 
                  className={`transition-transform duration-300 ${openDropdowns["treatments"] ? 'rotate-180' : ''}`}
                />
              </div>
              
              {openDropdowns["treatments"] && (
                <ul className="bg-gray-50">
                  {treatmentItems.map((item, index) => (
                    <li key={index} className="border-t border-gray-100">
                      <Link
                        href={item.path}
                        className="flex items-center px-9 py-3 text-gray-600 hover:text-blue-600 hover:bg-gray-100 transition-colors"
                        onClick={closeDrawer}
                      >
                        <span>{item.label}</span>
                      </Link>
