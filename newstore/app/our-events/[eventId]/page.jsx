...
import { displayMessage } from "@/dataarrange/utils/common";
...
export default function EventDetailsPage() {
    ...
    const registerForEvent = async () => {
        ...
        try {
            ...
            displayMessage(SUCCESS_MSG_TYPE, "Registration successful!");
    ...
    const verifyPaymentStatus = async (orderId) => {
        const maxAttempts = 15; // 30 attempts (2.5 minutes total)
        const retryDelay = 10000; // 5 seconds between attempts
        let attempts = 0;

        while (attempts < maxAttempts) {
            attempts++;
            setLoadingStatus(`Verifying payment (${attempts}/${maxAttempts})`);
    ...
    const checkOrderStatus = async (orderId) => {
        ...
        const response = await fetch(
            `https://healdiway.bkarogyam.com/erp-api/inv_order/${orderId}/`,
            {
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Token ${parsedToken}`
        ...
        if (orderData.order_status_data?.id === 3) {
            return 'success';
        }
        
        // Add condition for failed status if your API provides one
        // if (orderData.order_status_data?.id === X) return 'failed';
    ...
    const handlePaymentConfirmation = async (orderId) => {
        ...
        try {
            const paymentVerified = await verifyPaymentStatus(orderId);
            
            if (paymentVerified) {
                await registerForEvent();
                displayMessage(SUCCESS_MSG_TYPE, "Payment and registration successful!");
            } else {
    ...
    const initOrder = async () => {
        ...
        try {
            ...
            const orderData = {
                ...
                currency_code: 'Rs',
                shipping_firstname: userdata.user.first_name,
                shipping_address1: "Online Event",
                shipping_city: "Online",
                shipping_postcode: "000000",
                shipping_zone: "Online",
                order_status_data: { id: 2, name: "Initiated" },
                product_count: 1,
                is_active: true,
                customer_id: 33,
                customer_group: 1
            ...
            const response = await fetch(
                'https://healdiway.bkarogyam.com/erp-api/inv_order/',
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            setLoadingStatus('initiating-payment');
            await initiatePayment(responseData);
            
            handlePaymentConfirmation(responseData.id);
    ...
    const LoadingModal = ({ isOpen, status }) => {
        const getStatusMessage = () => {
            if (status.startsWith('Verifying payment')) {
                return (
...
//                 mobile: userdata.user.mobile || "",
//                 total: event.price,
//                 deliverycharge: 0, // No delivery charge for events
//                 order_status_id: 2, // Initiated status
//                 accept_language: 'English',
//                 currency_code: 'Rs',
//                 shipping_firstname: userdata.user.first_name,
//                 shipping_address1: "Online Event",
...
//                 shipping_postcode: "000000",
//                 shipping_zone: "Online",
//                 // These fields might be required based on the API response
//                 order_status_data: {
//                     id: 2,
//                     name: "Initiated"
//                 },
//                 product_count: 1, // Assuming 1 for event registration
//                 is_active: true,
//                 customer_id: 33 , // Make sure to include customer ID
//                 customer_group: 1 // Default group
//             };
...
//                         'Authorization': `Token ${parsedToken}`
//                     },
//                     body: JSON.stringify(orderData) // Send the data directly, not nested
//                 }
//             );
...
//             const responseData = await response.json();
//             initiatePayment(responseData); // Use the actual response data
//         } catch (error) {
